...
puzzle_input = """mmqyju (156) -> rjzvwv, noybkx
...
def part_1(puzzle, part_2=False):
    programs = dict()
    supported = list()
    for line in puzzle.splitlines():
        program, infos = line.split(" ", 1)
        programs[program] = {
            'weight': int(infos.split(")")[0][1:])
        }
        if " -> " in infos:
            support = (infos.split(" -> ", 1)[1]).split(', ')
            if part_2:
                programs[program]['supported'] = support
            else:
                supported += support


    """
    if yes, are they even
    if yes, change weight and add it to leaves
    if no, it's the one we need
    ...
    if part_2:
        leaves = list()
        others = list()
        for program, infos in programs.items():
            if 'supported' in infos:
                others.append(program)
            else:
        ...
        while len(others) > 0:
            for program in others:
                print(program)
                if programs[program]['supported'][0] in leaves:
                    weights = set()
                    for support in programs[program]['supported']:
                        weights.add(programs[support]['weight'])
                    weights = list(weights)
                    if len(weights) > 1:
                        # XXX need to substract this from initial weight
                        print(weights)
                        return weights[0] - weights[1]
                    else:
                        programs[program]['weight'] += weights[0] * len(programs[program]['supported'])
                        del programs[program]['supported']
                        leaves.append(program)
                        others.remove(program)
    else:
...
assert part_1(test) == 'tknk'
assert part_1(test, 'tknk') == 60

result = part_1(puzzle_input)
print(result)
print(part_1(puzzle_input, result))
