...
puzzle_input = """pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
...
def part_1(puzzle):
    result = 0
    for line in iter(puzzle.splitlines()):
        if no_duplicate(line):
            result += 1
...
for puzzle, expected in tests:
    assert no_duplicate(puzzle) == expected
...
for puzzle, expected in tests:
    assert no_anagram(puzzle) == expected
